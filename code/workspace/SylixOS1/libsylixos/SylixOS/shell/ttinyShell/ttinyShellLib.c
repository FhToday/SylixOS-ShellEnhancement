...
static INT  __tshellWrapper (FUNCPTR  pfuncCommand, INT  iArgC, PCHAR  ppcArgV[])
{
    ...
    if (ppcArgV[1][0] == '?' && ppcArgV[1][1] == '\0') {
        CPCHAR pcKeyword = ppcArgV[0];
        __PTSHELL_KEYWORD pskwNode = LW_NULL;
        REGISTER size_t stStrLen = lib_strnlen(pcKeyword, LW_CFG_SHELL_MAX_KEYWORDLEN);
...
//                printf(pskwNode->SK_pcHelpString);
//                __TTINY_SHELL_UNLOCK();                                     /*  释放资源                    */
//                return 0;
//            }

            __helpPrintKeyword(pskwNode, LW_NULL);
...
PVOID __tshellThread(PVOID pcArg) {
    ...
    if (__TTINY_SHELL_GET_OPT(ptcbCur) & LW_OPTION_TSHELL_AUTHEN) { /*  是否需要明文用户认证        */
        if (__TTINY_SHELL_GET_OPT(ptcbCur) & LW_OPTION_TSHELL_LOOPLOGIN) {
            __reauthen: if (__tshellUserAuthen(iTtyFd, LW_TRUE)) { /*  进行认证                    */
                goto __reauthen;
            }
        } else {
    ...
    if (__TTINY_SHELL_GET_OPT(ptcbCur) & LW_OPTION_TSHELL_NOECHO) { /*  不需要回显                  */
        iRetValue = ioctl(iTtyFd, FIOSETOPTIONS, (OPT_TERMINAL & (~OPT_ECHO))); /*  设置为终端模式              */
    } else {
    ...
    char temp[100];
    int templen = 0;
    PCHAR pipe = (PCHAR)__SHEAP_ALLOC(MAX_FILENAME_LENGTH);
    lib_strcpy(pipe, "/tmp/pipe");
    INT pipelen = lib_strlen(pipe);
    for (;;) {
...
//                    i++;
        ...
        if (iReadNum > 0) {
            ...
            if (cRecvBuffer[iTotalNum - 1] == '\\') { /*  用户需要续写命令            */
                 ...
                 */
                //printf("2\n");
                int iTotalNumtemp = iTotalNum;
                while (iTotalNumtemp > 0) {
                    if (cRecvBuffer[iTotalNumtemp - 1] == '|') {
                        //printf("3 templen: %d\n",templen);
                        inpipe = TRUE;
                        //将'|'后的命令存入char temp[100]里
                        int i = 0;
                        while (i < templen) {
                            temp[i] = cRecvBuffer[iTotalNumtemp + i];
                            i += 1;
                        }
                        temp[i] = ' '; //第二命令的keyword末尾添加空格
                        templen += 1;
                        temp[i + 1] = '\0';
                        //printf("3 temp: %s(end)\n",temp);
                        //printf("3 templen: %d\n",templen);
                        //将 | 变为 ">" + pipe:"pipe"
                        cRecvBuffer[iTotalNumtemp - 1] = '>';
                        iTotalNumtemp += 1;
                        i = 0;
                        while (i < pipelen) {
                            cRecvBuffer[iTotalNumtemp - 1] = pipe[i];
                            i++;
                            iTotalNumtemp += 1;
                        }
                        //在后面加上 PX_EOS('\0')
                        cRecvBuffer[iTotalNumtemp - 1] = PX_EOS;
                        iTotalNumtemp -= 1;
                        //printf("3 buffer: %s\n",cRecvBuffer);
                        //iTotalNum变化
                        iTotalNum = iTotalNumtemp;
                    } else {
                        iTotalNumtemp -= 1;
                        templen += 1;
                    }
