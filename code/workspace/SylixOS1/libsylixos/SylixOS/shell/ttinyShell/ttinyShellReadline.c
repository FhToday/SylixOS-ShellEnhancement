...
VOID completeCmd (INT iFd, PCHAR cmd, __PSHELL_INPUT_CTX psicContext) {
    CPCHAR Prefix[50];
    INT count = 0;
    INT i;
    for (i=0; i<cmdCount; i++) {
        if (isPrefix(cmd, cmdList[i]) == (1)) {
            Prefix[count] = cmdList[i];
            count++;
        }
    }
    if (count == 1) {
        CHAR cmdName[LW_CFG_SHELL_MAX_COMMANDLEN + 1];
        addBlack(Prefix[0], cmdName);
        INT stFileNameLen = lib_strlen(cmd);
        INT stCatLen = lib_strlen(cmdName) - stFileNameLen;
        write(iFd, &cmdName[stFileNameLen], stCatLen);
        lib_strlcat(CTX_BUFFER, &cmdName[stFileNameLen], LW_CFG_SHELL_MAX_COMMANDLEN);
        CTX_TOTAL += (UINT)stCatLen;
        CTX_CURSOR = CTX_TOTAL;
    }
    else if(count > 1) {
...
#define __TTINY_SHELL_CMD_ISEND(pcCmd)      (*(pcCmd) == PX_EOS)
    ...
    if (i == 0)  {
        completeCmd(iFd, pcDir, psicContext);
        return;
    }
    if (pcDir == LW_NULL) {
        return;
